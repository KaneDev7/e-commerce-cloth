...
import { commandsOfSelectedFilter, getTotalPriceOfPaidCommands, updateCommandsState } from '@/domain/use-case/command/command.useCase';
...
export default function AdminCommand() {
    ...
    const deleteCommandes = async () => {
        for (const id of itemsId) {
            await new CommandService().deleteCommand(id)
            setItemsId([])
        }
        toast.success(`les commandes supprimés avec succée `, {
    ...
    const handleSetCommand = async (item, event) => {
        ...
        await new CommandService().updateCommand(item.id, updateItem)
        const newCommandeState = updateCommandsState(commands, updateItem, item.id)
    ...
    useEffect(() => {
        (async function () {
            const commandsOfCurrentUser = await new CommandService().getCommands()

            if (filterSelected === 'tout') {
                setCommand(commandsOfCurrentUser)
            } else {
                const dataFilter = commandsOfSelectedFilter(commandsOfCurrentUser, filterSelected)
    ...
    useEffect(() => {
        const totalPrice = getTotalPriceOfPaidCommands(commands)
        setTotal(totalPrice)
    }, [commands])
